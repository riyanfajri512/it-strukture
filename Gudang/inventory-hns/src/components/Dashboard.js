...
function Dashboard({ user, onLogout }) {
  ...
  // 1. LOAD DATA LOOPING (Ambil semua data dari database)
  ...
  const processedProducts = useMemo(() => {
    ...
    if (status === "Ready") {
      result = result.filter((p) => parseFloat(p["Stok Sistem"]) > 0);
  ...
  const handleCheckAll = (e) => {
    ...
    const newCheck = {};
    if (isChecked) {
      processedProducts.forEach((p) => (newCheck[p["Kode Accurate"]] = true));
    }
    setChecked(newCheck);
  };
    ...
    <div className="dashboard-wrapper">
      ...
      <div className="filter-card no-print">
        ...
        <p style={{ marginTop: "10px", fontSize: "14px" }}>
          Total: <strong>{processedProducts.length}</strong> barang
          {Object.keys(checked).length > 0 && (
            <span style={{ marginLeft: "10px", color: "#3182ce" }}>
              | Dipilih: <strong>{Object.keys(checked).length}</strong>
            </span>
          )}
        </p>
      </div>
          ...
          <thead>
            <tr style={{ background: "#667eea", color: "white" }}>
              ...
              <th
                className="no-print"
                style={{ cursor: "pointer" }}
                onClick={() => requestSort("TANGGAL UPDATE")}
              >
                UPDATE {getSortIcon("TANGGAL UPDATE")}
              </th>
                ...
                >
                  ...
                  <td data-label="AKSI" className="no-print">
                    <div style={{ display: "flex", gap: "5px" }}>
                      {isEdit ? (
                        <button
                          onClick={() => handleSaveEdit(item["Kode Accurate"])}
                          style={{
                            background: "#38a169",
                            color: "white",
                            padding: "5px 10px",
                        ...
                        <>
                          <button
                            onClick={() => {
                              setEditingId(item["Kode Accurate"]);
                              setEditForm({
                                modal: item.CP,
                                srp: item.SP,
                                jual: item.PRICE,
                          ...
                          <button
                            onClick={() => handleDelete(item["Kode Accurate"])}
                            style={{
                              padding: "5px 10px",
                              border: "none",
                              background: "#e53e3e",
